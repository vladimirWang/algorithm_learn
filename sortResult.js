...
const bubbleData = generateRandomArray(Math.pow(10, 4))
...
const bubbleData6 = JSON.parse(JSON.stringify(bubbleData))
...
const mergeSortWorker = fork(path.join(__dirname, 'worker.js'))

// 处理第一个进程（基础版冒泡）
worker1.on('message', (msg) => {
    if (msg.success) {
        console.log("基础版冒泡用时: ", msToTime(msg.elapsed))
        console.log('排序前数据: ', bubbleData)
        console.log('排序后数据: ', msg.result)
    }
})
...
worker4.on('message', (msg) => {
    if (msg.success) {
        ...
        if (msg.result.length < 20) {
            console.log('baseQuick排序前数据: ', bubbleData4)
            console.log('baseQuick排序后数据: ', msg.result)
        }
    }
})
...
worker5.on('message', (msg) => {
    if (msg.success) {
        ...
        if (msg.result.length < 20) {
            console.log('quickSort排序前数据: ', bubbleData5)
            console.log('quickSort排序后数据: ', msg.result)
        }
    }
})
...
// 基础版快速排序
mergeSortWorker.on('message', (msg) => {
    if (msg.success) {
        console.log("mergeSort用时: ",  msToTime(msg.elapsed))
        if (msg.result.length < 20) {
            console.log('mergeSort排序前数据: ', bubbleData6)
            console.log('mergeSort排序后数据: ', msg.result)
        }
    }
})
...
mergeSortWorker.send({ algorithm: 'mergeSort', data: bubbleData6 })